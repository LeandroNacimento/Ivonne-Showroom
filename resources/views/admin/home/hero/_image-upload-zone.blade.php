...
<div>
    ...
    @if ($currentUrl)
        <div class="relative mt-2 overflow-hidden rounded-lg border border-gray-200 bg-gray-50">
            <img src="{{ $currentUrl }}"
                 alt="{{ $label }}"
                 class="{{ str_contains($inputName, 'mobile') ? 'aspect-[4/5]' : 'aspect-video' }} w-full object-cover">
    ...
    @else
        <div class="mt-2 flex {{ str_contains($inputName, 'mobile') ? 'aspect-[4/5]' : 'aspect-video' }} max-h-48 items-center justify-center rounded-lg border-2 border-dashed border-gray-300 bg-gray-50">
